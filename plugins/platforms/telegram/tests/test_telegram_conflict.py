...
def _no_auto_discovery(monkeypatch):
    ...
    async def _noop():
        return []
    monkeypatch.setattr("hermes_agent_telegram.adapter.discover_fallback_ips", _noop)
    # Mock HTTPXRequest so the builder chain doesn't fail
    monkeypatch.setattr("hermes_agent_telegram.adapter.HTTPXRequest", lambda **kwargs: MagicMock())
...
async def test_polling_conflict_retries_before_fatal(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent_telegram.adapter.Application", SimpleNamespace(builder=MagicMock(return_value=builder)))
...
async def test_connect_clears_webhook_before_polling(monkeypatch):
    ...
    builder.build.return_value = app
    monkeypatch.setattr(
        "hermes_agent_telegram.adapter.Application",
...
async def test_disconnect_skips_inactive_updater_and_app(monkeypatch):
    ...
    warning = MagicMock()
    monkeypatch.setattr("hermes_agent_telegram.adapter.logger.warning", warning)
